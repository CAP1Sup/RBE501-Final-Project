function [tau,V,Vdot] = mtm_rne(params, option)
...
%          option: 1 = No Added Torques, 2 = Friction Added, 3 = Spring
%          Added, 4 = Motor Inertia Added,  5 = Friction + Spring + Motor Inertia
%
...
%%  Adding elastic and friction torques MTM
...
K_e = diag([0.249, 0, 0, 0.042, -0.018, 0.003, 0]);
tau_e = K_e * params.jointPos';

% Friction torques
% Creating viscous friction matrix
Fv = diag([0.129, 0.119, 0.068, 0.066, 0.0001, 0.0001, 0.001]);
% Creating static friction matrix
Fs = diag([0.037, 0.053, 0.0001, 0.034, 0.011, 0.005, 0.0001]);
% Calculating torques per joint due to friction
tau_f = friction(params.jointVel', Fv, Fs, zeros(7,1));

% Calculating Torque due to Motor Inertia of Joint 4:
q3_ddot = params.jointAcc(3);
q4_ddot = params.jointAcc(4);
r3 = 14.01;
r4 = 20.92;
q4m_ddot = q4_ddot + r3/r4 * q3_ddot;
I4 = 0.02;
tau_MI4 = I4 * q4_ddot;
tau_MI = [0, 0, 0, tau_MI4, 0, 0, 0]';

gear_ratio_list = [63.41, 49.88, 59.73, 10.53, 33.16, 33.16, 16.58]'; %Corrected from [63.41, 49.88, 59.73, 10.53, 33.16, 64, 64]'
switch option
    case 1 % No Added Torques
        tau = tau ./gear_ratio_list;

    case 2 % Friction Added
        tau = tau + tau_f;
        tau = tau ./gear_ratio_list;
    ...
    case 4 % Motor Inertia Added:
        tau = tau ./gear_ratio_list;
        tau = tau - tau_MI;

    case 5 % Friction + Spring + Motor Inertia Added:
        tau = tau + tau_e + tau_f;
        tau = tau ./gear_ratio_list;
        tau = tau - tau_MI;
end
